...
BEGIN
	IF l = NIL THEN
		res := 0;
	ELSE
		res := 1 + ObtenerCantVuelosListaVuelos(l^.sig);
	END;
	RETURN(res);
END ObtenerCantVuelosListaVuelos;
...
BEGIN
	WHILE lVuelo <> NIL DO
		ImprimirInfoVuelo(arch,lVuelo^.v);
	END;
END ImprimirVuelosListaVuelos;
